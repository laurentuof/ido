...
void setup() {
  ...
  fonaSerial->begin(Serial_baud_rate,SERIAL_8N1,FONA_RX,FONA_TX);
  ...
  switch (type) { // determine le module FONA utiliser
    case FONA800L:
...
void loop() {
  ...
  switch (command) {
    ...
    case 'U': { // Unlock the SIM with a PIN code
        char PIN[5];
        flushSerial();
    ...
    case 'C': { // read the CCID
        fona.getSIMCCID(replybuffer);  // make sure replybuffer is at least 21 bytes!
        Serial.print(F("SIM CCID = ")); Serial.println(replybuffer);
        break;
      }
    case 'i': { // read the RSSI
...
*/
/*    case 'q': { // Get FM station signal level (in decibels).
        flushSerial();
        Serial.print(F("FM Freq (eg 1011 == 101.1 MHz): "));
        uint16_t station = readnumber();
        Serial.println();
...
*/
    ...
    /*** PWM ***/
/*    case 'P': { // PWM Buzzer output @ 2KHz max
        flushSerial();
...
*/
    ...
    /*** SMS ***/
    case 'N': { // read the number of SMS's!
    ...
    /*** Time ***/
/*    case 'y': { // enable network time sync
        if (!fona.enableNetworkTimeSync(true))
          Serial.println(F("Failed to enable"));
        break;
      }
*/
/*    case 'Y': { // enable NTP time sync
        if (!fona.enableNTPTimeSync(true, F("pool.ntp.org")))
          Serial.println(F("Failed to enable"));
        break;
      }
*/
/*    case 't': { // read the time
        char buffer[23];
...
*/
    ...
    case 'l': { // check for GSMLOC (requires GPRS)
        uint16_t returncode;
