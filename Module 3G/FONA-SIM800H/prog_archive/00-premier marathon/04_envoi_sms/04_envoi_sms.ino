...
String gsm_pin = "0000";    // remplacer par votre code PIN
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (a ==83 or a== 115) { // S ou s envi un SMS
      if (etat_gsm()) {
        gsm_envoi_sms("0000000000","Coucou !"); // remplacer 0000000000 par le numéro du téléphone visé
...
void gsm_envoi_sms(String tel, String msg) {
        ...
        GSM_Serial.println("AT+CMGS=\""+tel+"\"");      // remplacer par le numéro du téléphone auquel envoyer le SMS
        delay(100);
        // Envoi du texte
